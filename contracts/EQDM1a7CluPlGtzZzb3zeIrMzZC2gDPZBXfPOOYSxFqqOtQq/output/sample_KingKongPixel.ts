...
function initKingKongPixel_init_args(src: KingKongPixel_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeRef(src.content);
...
const KingKongPixel_types: ABIType[] = [
...
const KingKongPixel_getters: ABIGetter[] = [
...
const KingKongPixel_receivers: ABIReceiver[] = [
...
export class KingKongPixel implements Contract {
    
    static async init(owner: Address, content: Cell, max_supply: bigint) {
        return await KingKongPixel_init(owner, content, max_supply);
    }
    
    static async fromInit(owner: Address, content: Cell, max_supply: bigint) {
        const init = await KingKongPixel_init(owner, content, max_supply);
        const address = contractAddress(0, init);
        return new KingKongPixel(address, init);
    }
    
    static fromAddress(address: Address) {
        return new KingKongPixel(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  KingKongPixel_types,
        getters: KingKongPixel_getters,
        receivers: KingKongPixel_receivers,
        errors: KingKongPixel_errors,
    };
