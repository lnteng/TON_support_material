...
function initTonFarmToken_init_args(src: TonFarmToken_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeRef(src.content);
...
const TonFarmToken_types: ABIType[] = [
...
const TonFarmToken_getters: ABIGetter[] = [
...
const TonFarmToken_receivers: ABIReceiver[] = [
...
export class TonFarmToken implements Contract {
    
    static async init(owner: Address, content: Cell, max_supply: bigint) {
        return await TonFarmToken_init(owner, content, max_supply);
    }
    
    static async fromInit(owner: Address, content: Cell, max_supply: bigint) {
        const init = await TonFarmToken_init(owner, content, max_supply);
        const address = contractAddress(0, init);
        return new TonFarmToken(address, init);
    }
    
    static fromAddress(address: Address) {
        return new TonFarmToken(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  TonFarmToken_types,
        getters: TonFarmToken_getters,
        receivers: TonFarmToken_receivers,
        errors: TonFarmToken_errors,
    };
